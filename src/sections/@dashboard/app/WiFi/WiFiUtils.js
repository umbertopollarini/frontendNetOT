...
export default function WiFiUtils({ title, subheader, ...other }) {
        ...
        const handleRestartWiFi = () => {
            fetch('/wifi/restart', {
                ...
                .then(response => {
                    if (response.ok) {
                        setAlertSeverity('success');
                        setAlertMessage('WiFi restarted successfully');
                        setOpen(true);
                    } else {
                        setAlertSeverity('error');
                        setAlertMessage('WiFi restart failed');
                        setOpen(true);
                    }
                })
                .catch(error => {
        ...
        return (
            <Card {...other}>
                <CardHeader title={"WiFi Utils"} subheader={subheader} />
                <Box p={3}>
                    <TextField
                        id="config"
                    ...
                    <Box mt={2}>
                        <Button variant="contained" onClick={handleRestartWiFi}>Restart WiFi</Button>
                    </Box>
                    <Snackbar
                        open={open}
